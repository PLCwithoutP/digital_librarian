...
def make_bibtex_record(rec: Dict[str, Any], used_keys: Dict[str, int]) -> Dict[str, Any]:
    ...
    # If parser found a container title ("journal" field), decide how to map it.
    # - Default: treat as @article if we otherwise would have used @misc.
    # - If the container looks like a proceedings/event (Forum/Conference/Symposium/etc.), use @inproceedings.
    journal_lc = _fold_ascii(journal).lower()
    looks_like_event = any(k in journal_lc for k in [
        "forum", "conference", "symposium", "workshop", "proceedings", "meeting", "congress",
    ])
    if journal and entry_type == "misc":
        entry_type = "inproceedings" if looks_like_event else "article"
    ...
    if journal:
        if entry_type == "inproceedings":
            # BibTeX expects booktitle for conference papers
            fields.setdefault("booktitle", journal)
        else:
            fields["journal"] = journal
    if volume is not None and str(volume).strip():
        fields["volume"] = str(volume).strip()
    if number is not None and str(number).strip():
        fields["number"] = str(number).strip()
